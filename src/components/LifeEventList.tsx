...
function getTypeColor(type: LifeEventType): string {
  switch (type) {
    case 'expense':  return '#f87171';
    case 'income':   return '#4ade80';
    case 'lump_sum': return '#fb923c';
  }
}

function getTypeLabel(type: LifeEventType): string {
  switch (type) {
...
export function LifeEventList({ lifeEvents, profile, onAdd, onUpdate, onDelete }: LifeEventListProps) {
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          <h4 className="text-md font-medium text-gray-800 dark:text-gray-200 mb-3">
            {editingEvent ? 'Edit Life Event' : 'New Life Event'}
          </h4>
          <LifeEventForm
      ...
      {lifeEvents.length === 0 && !showForm ? (
        <p className="text-gray-500 dark:text-gray-400 text-sm py-4 text-center">
          No life events added yet. Click '+ Add' to model one-time or time-bounded cashflow events like college tuition, home purchase, or inheritance.
        </p>
      ) : (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    {getTypeLabel(event.type)} · Age {event.startAge}{event.endAge ? `–${event.endAge}` : ''}
                    {event.inflationAdjust && <span className="ml-1 opacity-70">· inflation-adj.</span>}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    {event.type === 'lump_sum' ? 'one-time' : '/yr'}
                  </div>
                </div>
